...
FIREBASE_CONFIG = {
    "apiKey": "AIzaSyBw7MuFcrYrXHUz1wr0YikSX6JFraBRAo0",
    "authDomain": "pkd-robo-ai.firebaseapp.com",
    "databaseURL": "https://pkd-robo-ai-default-rtdb.firebaseio.com",
    "projectId": "pkd-robo-ai",
    "storageBucket": "pkd-robo-ai.firebasestorage.app",
    "messagingSenderId": "66636477959",
    "appId": "1:66636477959:web:3fa6eddd3217faddecdca2",
}
...
# We use the REST API approach for Firebase Realtime Database so we don't
# need a service-account key file.  All DB operations go through the
# public REST endpoint (the database rules must allow read/write).
# ---------------------------------------------------------------------------
...
def _fb_url(path: str) -> str:
    """Return full Firebase REST URL for a given path."""
...
def fb_patch(path: str, data):
    """PATCH (update) data in Firebase Realtime Database."""
    try:
...
def _ensure_default_features():
    """Make sure every feature key exists in Firebase."""
    existing = fb_get("admin/features")
    if not existing:
        existing = {}
    for key in FEATURE_KEYS:
        if key not in existing:
            existing[key] = True  # default ON
...
def save_user(user):
    """Save or update user info in Firebase."""
    uid = str(user.id)
    data = {
        "user_id": user.id,
...
def get_chat_history(user_id: int) -> list:
    """Return list of past messages for context."""
    uid = str(user_id)
...
def query_ai(user_id: int, user_message: str) -> str:
    """Send message to the AI API with system prompt + chat history context."""
    try:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_group:
        ...
        # Remove trigger words to get actual query
        cleaned = lower_text
        for t in triggers:
            cleaned = cleaned.replace(t, "")
        cleaned = cleaned.strip()
        if not cleaned:
            cleaned = message_text  # fallback to original
